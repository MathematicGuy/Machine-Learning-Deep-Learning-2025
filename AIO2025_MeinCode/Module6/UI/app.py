...
def load_training_data():
    ...
    if os.path.exists("Bank_train2.csv"):
        df = pd.read_csv("Bank_train2.csv")
...
def load_realdata(start_date=None, end_date=None):
    ...
    if os.path.exists("Bank_realdata2.csv"):
        try:
            df = pd.read_csv("Bank_realdata2.csv")
...
def main():
    ...
    if train_df is None or forecast_df is None:
        ...
        st.info("Expected files: Bank_train2.csv, Bank_forecast.csv")
        return
    ...
    with tab1:
        ...
        if chart_type == "Candlestick":
            ...
            chart_subtitle = f"*{timeframe_desc[timeframe]} candlesticks - Complete Open, High, Low, and Close data from training set Bank_train2.csv"
        else:
        ...
        with col1:
            st.metric("Current Price", f"{current_price:,.2f} USD", help="Last known price from training data")
        with col2:
            st.metric("100-Day Forecast", f"{forecast_final:,.2f} USD", f"{forecast_change:+.2f}%")
        with col3:
            if 'uncert_lower_ana' in forecast_df.columns:
                lower_bound = forecast_df['uncert_lower_ana'].iloc[-1]
                st.metric("Lower Bound (90%)", f"{lower_bound:,.2f} USD")
        with col4:
            if 'uncert_upper_ana' in forecast_df.columns:
                upper_bound = forecast_df['uncert_upper_ana'].iloc[-1]
                st.metric("Upper Bound (90%)", f"{upper_bound:,.2f} USD")
        ...
        if show_real_overlay and real_df is not None:
            ...
            if mse_result and mse_result['count'] > 0:
                ...
                with col2:
                    st.metric("RMSE", f"{mse_result['rmse']:,.2f} USD")
                with col3:
                    st.metric("MAE", f"{mse_result['mae']:,.2f} USD")
                with col4:
        ...
        if forecast_df2 is not None:
            ...
            with col1:
                st.metric("Current Price", f"{current_price2:,.2f} USD", help="Last known price from training data")
            with col2:
                delta_vs_original = forecast_final2 - forecast_final
                st.metric(
                    "Custom Forecast",
                    f"{forecast_final2:,.2f} USD",
            ...
            with col3:
                if 'uncert_lower_ana' in forecast_df2.columns:
                    ...
                    st.metric("Lower Bound (90%)", f"{lower_bound2:,.2f} USD")
            with col4:
                if 'uncert_upper_ana' in forecast_df2.columns:
                    upper_bound2 = forecast_df2['uncert_upper_ana'].iloc[-1]
                    st.metric("Upper Bound (90%)", f"{upper_bound2:,.2f} USD")
            ...
            if show_real_overlay and real_df is not None:
                ...
                if mse_custom and mse_custom['count'] > 0:
                    ...
                    with col2:
                        st.metric("RMSE", f"{mse_custom['rmse']:,.2f} USD")
                    with col3:
                        st.metric("MAE", f"{mse_custom['mae']:,.2f} USD")
                    with col4:
            ...
            # Comparison info
            st.info(f"💡 **Comparison:** Original forecast: {forecast_final:,.2f} USD | Custom forecast: {forecast_final2:,.2f} USD | Difference: {delta_vs_original:+.2f} USD ({((delta_vs_original/forecast_final)*100):+.2f}%)")
